...
function inject(bot) {
  ...
  var CHUNK_BITS = 4;
  function chunkColumnArray(x, y, z) {
    if ((x & ~CHUNK_MASK) !== x || (y & ~CHUNK_MASK) !== y || (z & ~CHUNK_MASK) !== z)
      throw new Error('must start at corner of chunk');
    var key = columnKeyXZ(x, z);
    var column = columns[key];
    if (! column) return null;

    var buf = column.blockType[y >> CHUNK_BITS];
    if (!buf) return null; // air minichunk
  ...
  bot.client.on('update_sign', function(packet) {
    ...
    var str = new ChatMessage(JSON.parse(packet.text1)) + "\n" + new ChatMessage(JSON.parse(packet.text2)) +
  ...
  bot.blockAt = blockAt;
  bot.chunkColumnArray = chunkColumnArray;
